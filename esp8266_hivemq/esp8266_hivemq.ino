...
const char* mqtt_pub_topic   = "esp8266/status";
const char* mqtt_sub_topic   = "esp8266/control";
const char* mqtt_button_topic = "esp8266/button";
...
void setup() {
  ...
  pinMode(BUTTON_PIN, INPUT);   // nhớ có R pull-down thật!
...
void loop() {
  ...
  // Gửi trạng thái LED + ADC đều đặn
  ...
  // Xử lý nút nhấn vật lý (gửi sự kiện lên web)
  static unsigned long lastDebounce = 0;
  bool btnState = digitalRead(BUTTON_PIN);
  if (btnState == HIGH && lastBtnState == LOW && millis() - lastDebounce > 200) {
    // Phát hiện nhấn (chống dội 200ms, đổi tuỳ nút)
    lastDebounce = millis();
